...
export default function BanningFragment({ params }: { params: { socket: Socket, battleId: string, address: string, client2_address: string } }) {
    ...
    return (
        <div className="w-full h-screen flex flex-row justify-between p-10">
            <div className="w-full max-w-[800px] h-[650px] flex flex-col justify-center items-center">
                <p className="text-4xl font-black text-yellow-50 shadow-md">{"Guild 1"}</p>
                    ...
                    <div className="w-fit h-full flex flex-wrap justify-center items-center overflow-y-auto ">
                        {
                            client1Axies.map((e, index)=>(
                                <div key={index} className="w-fit flex flex-col justify-center items-center m-1 bg-fg-item p-2 border-fg-shadow border-4 rounded-xl shadow-xl">
                                    <div className="w-full h-[150px] bg-sm-bg rounded-lg p-2 flex flex-col justify-center items-center">
                                        <Image src={`https://assets.axieinfinity.com/axies/${e}/axie/axie-full-transparent.png`} width={120} height={120} alt="" unoptimized={true} className="inline-block" />
                                    </div>
                                    <div className="w-fit flex flex-row justify-normal items-center">
                                        <p className="text-white font-semibold mr-2">{`Axie #${e}`}</p>
                                    </div>
                                </div>    
                            ))
                        }
            ...
            <div className="w-full max-w-[800px] h-[650px] flex flex-col justify-center items-center">
                <p className="text-4xl font-black text-yellow-50 shadow-md">{"Guild 2"}</p>
                <div className="w-full h-full bg-fg-shadow mt-5 p-5 rounded-lg shadow-lg  flex flex-col justify-center items-start ">
                    <div className="w-fit h-full flex flex-wrap justify-center items-center overflow-y-auto ">
                        {
