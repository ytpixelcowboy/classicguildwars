...
export default forwardRef<Ref, Props>(function DefaultMediumModal({ onClose, callback}, ref) {
    ...
    return (
        <dialog ref={ref} className="w-[500px] bg-bg rounded-[15px] border-fg-item border-2">
            ...
            <div className="w-full h-fit">
                <div className="p-5 bg-fg">
                    ...
                    <p className="text-base font-bold text-white mt-2">Guild 2 address: </p>
                    <input className="w-full font-sans bg-sm-bg-light text-black p-3 border-solid border-item-bd border-2 rounded-lg mr-2" type="text" min={1} onChange={(event) => {
                        setClient2Address((event.target.value).toString());
                    }} />
                    <div className="w-full flex flex-row justify-end">
                        <button className="h-[50px] w-[200px] pl-6 pr-6 pt-2 pb-2 bg-fg border-bg text-white rounded-item-bd drop-shadow-lg disabled:bg-gray-600 disabled:text-gray-400 border-4 rounded-lg mt-10" onClick={() => {
